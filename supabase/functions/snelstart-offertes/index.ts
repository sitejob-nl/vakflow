...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (req.method) {
      case "GET": {
        if (id) {
          const data = await snelstartFetch(admin, conn, `/offertes/${id}`);
          return jsonRes(data);
        }
        const filter = url.searchParams.get("filter") || undefined;
        const data = await snelstartGetAll(admin, conn, "/offertes", filter);
        return jsonRes(data);
      }
      case "POST": {
        const body = await req.json();
        const data = await snelstartFetch(admin, conn, "/offertes", "POST", body);
        return jsonRes(data, 201);
      }
      case "PUT": {
        if (!id) return jsonRes({ error: "id is required" }, 400);
        const body = await req.json();
        const data = await snelstartFetch(admin, conn, `/offertes/${id}`, "PUT", body);
        return jsonRes(data);
      }
      case "DELETE": {
        if (!id) return jsonRes({ error: "id is required" }, 400);
        await snelstartFetch(admin, conn, `/offertes/${id}`, "DELETE");
    ...
    console.error("snelstart-offertes error:", err);
